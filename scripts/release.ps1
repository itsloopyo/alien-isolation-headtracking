...
# Fully unattended release workflow for AlienIsolationHeadTracking.
...
# release runs end to end with zero prompts. The preconditions below (valid
# semver, on main, clean tree, tag absent) are the safety net in place of any
...
# AIHT_VERSION is what the .asi logs at attach; MOD_VERSION is what install.cmd
# writes into .headtracking-state.json; launcher-manifest.json's
# mod_info.version is what the launcher reads. All must move together or
# shipped artifacts report a stale version.
$versionFiles = @(
    @{ Path = 'CMakeLists.txt';                             Pattern = "project\($modName VERSION \d+\.\d+\.\d+"; Replacement = "project($modName VERSION $target" },
    @{ Path = 'pixi.toml';                                  Pattern = '(?m)^version = "\d+\.\d+\.\d+"';          Replacement = "version = `"$target`"" },
    @{ Path = 'src/AlienIsolationHeadTracking/version.h';   Pattern = '#define AIHT_VERSION "\d+\.\d+\.\d+"';    Replacement = "#define AIHT_VERSION `"$target`"" },
...
git -C $ProjectRoot add CMakeLists.txt pixi.toml src/AlienIsolationHeadTracking/version.h scripts/install.cmd launcher-manifest.json CHANGELOG.md
